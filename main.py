...
# Слова необхідно шукати у всіх регістрах. Тобто. якщо необхідно знайти
# слово "one", значить для нього будуть підходити слова "one", "One", "oNe", "ONE" і т.д.
...
def popular_words (text, words):
   ...
   my_dict={}
   while i<=len(words)-1:
       my_dict.update({words[i]: text.count(words[i])})
       i=i+1
   return my_dict
